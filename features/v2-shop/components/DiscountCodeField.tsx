...
const PILL = 'flex min-w-0 flex-1 items-center rounded-pill bg-white px-4 py-2.5 text-sm leading-normal'
const APPLY = 'shrink-0 rounded-pill px-5 py-2.5 text-sm font-medium leading-normal transition-colors disabled:opacity-50'
...
export function DiscountCodeField({ state, value, onChange, onApply, onClear, savedText, errorText, busy = false }: DiscountCodeFieldProps) {
  const isError = state === 'error'
  const isSuccess = state === 'success'

  return (
    <section data-testid="discount-field" data-state={state} className={CARD} aria-label={DISCOUNT_LABEL}>
      <div className="flex items-center gap-2">
        {/* 🔥 is decoration in the design (55162:352) — aria-hidden so a screen reader reads the label once. */}
        <span aria-hidden className="text-base leading-none">🔥</span>
        <span className="text-[15px] font-medium text-v3-sapphire">{DISCOUNT_LABEL}</span>
      </div>
