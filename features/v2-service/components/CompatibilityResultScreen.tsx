...
export function CompatibilityResultScreen({ matchingId }: { matchingId: string }) {
  ...
  return (
    <div data-testid="compat-result-screen" data-state="ready" className="relative min-h-screen w-full overflow-x-hidden bg-v3-bg-cream font-ibm">
      ...
      <ResultActionBar shareText="ผลดวงสมพงศ์ของฉันจาก Mumate" testIdPrefix="compat" og={{ title: shareTitle, subtitle: shareSubtitle, summary: shareSummary, tag: "ผลความสมพงศ์", image: shareImages[0] }} getShareFile={() => captureShareImage(shareCardRef.current)} />
