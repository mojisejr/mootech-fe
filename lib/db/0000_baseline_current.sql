-- ⚠️ BASELINE — REFERENCE ONLY, DO NOT RUN. (#mootech-drizzle-rebaseline, 2026-06-25)
-- This file is the drizzle re-baseline of the ALREADY-EXISTING live DB (a pgloader'd MySQL copy).
-- It exists so meta/0000_snapshot.json ≡ schema.ts ≡ live DB and `drizzle-kit generate` diffs
-- cleanly. The tables below already exist in dev + prod — running this would error / be wrong.
-- Real migrations are hand-authored and applied manually (see lib/db/_archive/0001_*.sql for the
-- pattern). See the workflow contract at the top of lib/db/schema.ts.
...
--> statement-breakpoint
CREATE INDEX "idx_fortune_telling_log_user_id" ON "fortune_telling_log" USING btree ("user_id" text_ops);--> statement-breakpoint
...
CREATE INDEX "idx_member_with_friend_user_id" ON "member_with_friend" USING btree ("user_id" text_ops);--> statement-breakpoint
