...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!userId) {
      // Signed in, in the preview, but never completed register-login on this deployment.
      return res.status(404).json({ ok: false, error: 'no account for this login yet' })
    }
    ...
    // Never leak the DB error text to a browser; the server log keeps the detail.
