...
| บอร์ดพร่อง (ทิศซ้ำ — ค่าที่ fixture เคยส่งก่อนใบนี้ซ่อม) | `harness/out/md-daydetail-partial-board-393.png` |
...
## ⭐ ของที่ใหญ่ที่สุดของใบนี้ — **เข็มทิศหมุนทุกวัน**

อ่านตารางต้นทาง bazi **209 แถวสมบูรณ์** (ไม่ใช่ payload ใบเดียว):
```
ลำดับ GLYPH     = คงที่ 1 แบบ ทั้ง 209 แถว  →  開 休 生 傷 杜 景 死 驚  (ลำดับตามตำรา)
ลำดับ DIRECTION = 8 แบบ                     →  ทิศเดิม แต่ "หมุน"
```
⇒ **array เรียงคงที่จริง แต่เรียงตามอักษรประตู ไม่ใช่ตามทิศ**
⇒ วางด้วย index = ตรึงอักษรไว้ช่องเดิมตลอดกาล ⇒ **ถูก 1 ใน 8 · ผิด 7 ใน 8 · และดูถูกทุกวัน** เพราะป้ายเดินไปกับอักษร

> **180° ที่วัดได้ตอนเช้าไม่ใช่เบาะแส มันคือเศษซากของสิ่งที่ใหญ่กว่า**
> ไม่ใช่กระจกลึกลับ — **มันคือวงล้อที่หมุนทุกวัน** ผมบังเอิญจับตอนมันหมุนครบครึ่ง
> **ถ้าเจอวันที่หมุน 1 ช่อง ผมอาจไม่สังเกตเลย และเราคงชิปไปแล้ว**

**ของจริงมี 2 รูปเท่านั้น**: `gates="NE"` · `lucky_dir="ทิศ SE"`
รูปที่ 3 (`ทิศตะวันออกเฉียงเหนือ`) โผล่ใน repo แค่ 3 ที่ และ**ปลอมทั้งหมด** (fixture + test 2 ไฟล์)
⇒ **ตัดสาขาที่ 3 ออกจาก parser** (ตู๋) — สาขาที่ไม่มีผู้ผลิตจริง **ไม่ได้เพิ่มความยืดหยุ่น มันเปลี่ยนล้มดังเป็นผ่านเงียบ**
⇒ ซ่อมของปลอมทั้ง 3 ที่ให้เป็นค่าจริง · **อะไรที่อ่านไม่ออก โผล่เป็นกล่องเตือน ไม่หายเงียบ**

## adversary sign-off

**สิ่งที่พยายามหักล้างของตัวเอง**

1. **🔴 กับดัก mock — และผมแก้ผิดทางรอบแรก** เห็น `fixtures.ts` ส่งไทยยาว เลยขยาย parser ให้รับ ⇒ **ตู๋ทักว่าไม่มีผู้ผลิตจริงสักราย** ⇒ ผมกำลังขยาย parser เพื่อให้**ของปลอมที่ผิดผ่านได้** ⇒ ตัดสาขาออก แล้ว**ซ่อมของปลอมแทน** · บทเรียน: **เจอ mock ไม่ตรงของจริง ให้ซ่อม mock ไม่ใช่ยืดโค้ดจริงไปรับมัน**
