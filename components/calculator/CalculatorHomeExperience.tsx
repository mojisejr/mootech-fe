// Shared calculator experience (#calculator-homepage-swap). The public Bazi Calculator's full
// surface — teal canvas + gated header + phase machine (hero → form → ritual → result) — extracted
// so BOTH the homepage (pages/index.tsx) and pages/calculator/index.tsx render the exact same
// experience. Each page owns its own <Head> (title/robots) and getServerSideProps (the compute
// nonce cookie); this component is the view + client logic only.
//
// The compute API rejects requests without a valid nonce cookie (lib/calculator/nonce.ts) — issued
// by each host page's getServerSideProps — which blocks direct scripted POSTs invisibly (no captcha).
        ...
        {phase === 'form' && (
          <>
            ...
            <section className="flex min-h-[calc(100vh-60px)] flex-col text-white lg:flex-row lg:items-center lg:justify-center lg:gap-10 lg:px-10">
              ...
              {/* mascot — full-width bottom anchor on mobile (layout A); on desktop capped ~320px
                  (like index's 322×420) so it sits proportionately beside the hero, not dominant. */}
              <div className="mt-6 w-full lg:mt-0 lg:w-auto lg:flex-none">
                <Image
                  ...
                  alt=""
                  aria-hidden="true"
                  className="mx-auto h-auto w-full max-w-md object-contain lg:w-[320px] lg:max-w-none"
                  priority
                />
              </div>
            </section>
