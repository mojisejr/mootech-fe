...
function GlyphBox({ slot, isDay }: { slot: PillarSlot; isDay: boolean }) {
  if (!slot) {
    return (
...
export function PillarGrid({ columns, reveal = true }: { columns: PillarColumn[]; reveal?: boolean }) {
    ...
    <div className="mx-auto grid w-full max-w-md grid-cols-5 gap-2 lg:max-w-2xl lg:gap-4" data-testid="pillar-grid">
      {columns.map((col, i) => (
        ...
        >
          ...
          <GlyphBox slot={col.above} isDay={col.isDay} />
