...
### Semantic scale — GRADE (13 levels · **5 colour zones**) `✓`

**สายส่งมา 13 ระดับ · สีพูด 5 โซน · ตัวอักษรเกรดแบกความละเอียดที่เหลือ**
`lib/v2/grade-scale.ts` — ที่เดียว ใช้ร่วมกับ ดวงสมพงศ์ · ค่าสุ่มจาก Figma `636:19532`

| โซน | Card bg | Accent/badge | ink บนป้าย | เกรดที่อยู่ในโซนนี้ |
|---|---|---|---|---|
| best | `#E8F5E9` | `#2E7D32` | `#FFFFFF` | **A+ · A · A-** |
| good | `#F0F8F0` | `#66BB6A` | `#FFFFFF` | **B+ · B · B-** |
| fair | `#F9FBE7` | `#CDDC39` | `#374151` ⚠️ dark (contrast exception) | **C+** |
| weak | `#FFF0E1` | `#F57C00` | `#FFFFFF` | **C · C-** |
| poor | `#FCE4EC` | `#B71C1C` | `#FFFFFF` | **D+ · D · D- · F** |

#### ทำไมยุบจาก 10 สี เหลือ 5 โซน — **เพราะแยกไม่ออก ไม่ใช่เพราะยืมสีมาใช้**

แถบ 10 ขั้นเดิมวัดด้วย ΔE2000 (JND ≈ 2.3 · ต่ำกว่า ~10 = ต้องเพ่ง):

| | คู่ที่ติดกันซึ่งต่ำกว่า ΔE 10 |
|---|---|
| ตาปกติ | **3 / 9** |
| ตาบอดสีเขียว (deuteranopia) | **6 / 9** |
| ตาบอดสีแดง (protanopia) | **5 / 9** |

แย่สุด `B-` → `C+` คนตาบอดสีแดงเห็น **ΔE 2.9** — คือสีเดียวกัน
ชุดย่อยที่ใหญ่ที่สุดของแถบนี้ ซึ่งทุกคู่ติดกันผ่าน ΔE 10 **ในทุกแบบสายตา** = **5 ขั้น**
⇒ 10 ขั้นก็เกินตัวอยู่แล้ว · 13 ไม่มีทางอ่านออก

แถบ 5 โซนนี้วัดแล้ว: คู่ติดกันต่ำกว่า ΔE 10 = **0/4** · แย่สุดของ**ทุกคู่** (การ์ดโชว์ 4 ด้านพร้อมกัน ไม่เรียงตามสเกล) = `best↔poor` **ΔE 13.3** ยังผ่าน

#### ⚠️ ความละเอียดไม่ได้หายไป — มันย้ายที่

| | สีแบก | แยกออกจริง | ตัวอักษรแบก |
|---|---|---|---|
| เดิม | 10 ระดับ | ~5 | ของแถม |
| **ใหม่** | 5 โซน | **5 ครบ** | **13 ระดับเต็ม** |

ผู้ใช้ได้ความละเอียด **มากขึ้น** (13 > 10) และคราวนี้สีครึ่งหนึ่งของมันพูดความจริง
ทุกที่ที่ทาสีจากสเกลนี้ **มีตัวอักษรเกรดพิมพ์กำกับเสมอ** (`GradeBadge` · `CompatList`/`PredictionCards` เรนเดอร์ผ่าน `GradeBadge`)
ข้อยกเว้นเดียว: แถบ bar ใน `CompatList` เป็นสีล้วนไม่มีตัวอักษรบนตัวมัน — แต่ป้ายที่มีตัวอักษรอยู่แถวเดียวกัน

> **ทำไมต้องอ่านออกจริง**: ข้อมูลจริงรายด้าน (13 วัน 2 คน) พบว่า **69–88% ของวัน** มี 2 เกรดที่**ติดกันบนสเกล**อยู่ในการ์ดใบเดียวกัน — แถบที่อ่านไม่ออกจะโผล่ให้เห็นเกือบทุกวัน ไม่ใช่นานๆ ที

#### 🔀 สเกลนี้ **แยกจาก** CALENDAR day-cell โดยตั้งใจ
...
ที่ยอมรับได้เพราะ **`MonthGrid` ทาช่องของวันที่เลือกด้วยแซฟไฟร์ `SELECTED.fill` ทับสี tier** ⇒ **วันเดียวกันไม่เคยถูกทาสองระบบพร้อมกัน โดยโครงสร้าง**
และถ้าเอาเขียว-เหลือง-ส้ม-แดงไปทา 30 ช่องพร้อมกัน ตารางจะกลายเป็นต้นคริสต์มาส — ฟ้า/ส้ม/แดง 3 ระดับอ่านเป็น heat map ได้สงบกว่า ซึ่งเป็นเหตุผลที่ Figma วาดแบบนั้น
